...
dtfApp.config([
  ...
  function ($routeProvider) {
    $routeProvider.when('/', { templateUrl: 'views/home.html' }).when('/Intergration Jobs', { templateUrl: 'views/organizations_tree.html' }).otherwise({ templateUrl: 'views/404.html' });
  }
]);dtfApp.controller('MainController', [
  '$scope',
  '$http',
  '$timeout',
  '$location',
  function ($scope, $http, $timeout, $location) {
    ...
    $scope.jobs = [
      ...
      {
        'label': 'Intergrations',
        'children': [{ 'label': 'Intergration Jobs' }]
      },
      {
        'label': 'Reporting',
        'children': [{}]
    ...
    $scope.toggleSelect = function (prop) {
      ...
      var selectedProp = $scope.selectedItem.prop;
      //column
      if (level == '') {
